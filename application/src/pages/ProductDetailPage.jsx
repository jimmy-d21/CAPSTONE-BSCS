...
export function ProductDetailPage() {
  ...
  const drinksTotal = selectedDrinks.reduce(
    (s, id) => s + (DRINKS.find((d) => d.id === id)?.price ?? 0),
    0,
  );
  ...
  const relatedItems = menuData
    .filter(
      (m) => m.category === item.category && m.id !== item.id && m.available,
    )
    .slice(0, 4);
    ...
    <div style={{ background: "var(--bg)", minHeight: "100dvh" }}>
      ...
      <div style={{ padding: "0 0 calc(var(--tab-h) + 100px)" }}>
        ...
        <div style={{ padding: "20px 16px 0" }}>
          ...
          <Badge color="neutral" style={{ marginBottom: "10px" }}>
            {item.category}
          </Badge>
          <p
            style={{
        ...
        {item.customizable && item.customOptions?.length > 0 && (
          <Section
            ...
            subtitle="Select your preferred cut"
            required
          >
            <div
            ...
            >
              {item.customOptions.map((opt) => (
                <OptionRow
                  key={opt}
                  label={opt}
                  selected={selectedCut === opt}
                  onSelect={() => setSelectedCut(opt)}
                />
              ))}
            </div>
          </Section>
        ...
        <Section
          title="Add a Drink"
          subtitle="Select one or more drinks to go with your order"
        >
        ...
        <Section
          title="Add Extras"
          subtitle="Extras and add-ons to complete your meal"
        >
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {EXTRAS.map((extra) => (
              <AddonRow
        ...
        >
          ...
          {(selectedDrinks.length > 0 || selectedExtras.length > 0) && (
            <div style={{ textAlign: "right" }}>
              ...
              >
                Base: {formatPrice(item.price * qty)}
              </p>
              {drinksTotal > 0 && (
                <p
                  style={{
                ...
                >
                  Drinks: +{formatPrice(drinksTotal * qty)}
                </p>
              )}
              {extrasTotal > 0 && (
                <p
                ...
                >
                  Extras: +{formatPrice(extrasTotal * qty)}
                </p>
              )}
            </div>
          )}
